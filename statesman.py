...
class Action(pydantic.BaseModel):
    ...
    @classmethod
    def _cache_signature(cls, values: Any) -> Any:
        if isinstance(values, dict) and (callable := values.get("callable", None)):
...
class State(BaseModel):
    ...
    def add_action(self, callable: Callable, type_: Literal[Action.Types.entry, Action.Types.exit]) -> Action:
        ...
        if type_ not in acceptable_types:
            raise ValueError(
                f"cannot add state action with type \"{type_}\": must be {
                    _summarize(
                        acceptable_types,
                        conjunction='or',
                        quote=True)}",
    ...
    def remove_actions(
        self,
        actions: Union[None, List[Action], Literal[Action.Types.entry, Action.Types.exit]] = None,
    ) -> None:
...
class Event(BaseModel):
    ...
    def get_actions(
        self,
        type_: Literal[Action.Types.guard, Action.Types.before, Action.Types.after],
    ) -> List[Action]:
...
class StateMachine(pydantic.BaseModel):
        ...
        **kwargs,
        ...
        if isinstance(state, State):
            if state not in self._states:
                raise ValueError("invalid initial state: the state object given is not in the state machine")
            self._state = state
        ...
        if state_:
            with _state_entry(state_machine):
                if not await state_machine.enter_state(state_, *args, **kwargs):
                    raise RuntimeError("failed creation of state machine: could not enter the requested state")
    ...
    def remove_state(self, state: State) -> None:
        ...
        if state == State.active():
            raise ValueError("cannot remove the active State")
    ...
    async def trigger_event(
        self, event: Union[Event, str], *args, return_type: Optional[Type[Result]] = None, **kwargs
        ...
        if self.state is None and target is None:
            raise RuntimeError("event trigger failed: cannot transition from a None state to another None state")
...
class Transition(pydantic.BaseModel):
    ...
    async def __call__(self, *args, **kwargs) -> bool:
        ...
        if self.started_at:
            raise RuntimeError("transition has already been executed")
        ...
        async with self._lifecycle():
            ...
            try:
                ...
                if result not in (True, False, None):
                    raise ValueError("invalid return value from guard_transition: must return True, False, or None")
                if result is False:
                    raise AssertionError("transition cancelled by guard_transition callback")
            ...
            try:
                ...
                def _reduce_guard_results(x: Any, y: Any) -> bool:
                    if y not in (True, False, None):
                        raise ValueError("invalid return value from guard action: must return True, False, or None")
                    return x and (y in (True, None))

                results = await self._run_actions(self.event, Action.Types.guard, concurrently=False)
                success = functools.reduce(_reduce_guard_results, results, True) if results else True
                if not success:
    ...
    @pydantic.model_validator(mode="before")
    @classmethod
    def _set_default_type_from_event(cls, values: Any) -> Any:
    ...
    async def _run_actions(
        self, model: Optional[BaseModel], type_: Action.Types, concurrently: bool = True, **kwargs
    ) -> Optional[List[Any]]:
        ...
        """
        return (
            await model._run_actions(
                type_, transition=self, *self.args, concurrently=concurrently, **{**self.kwargs, **kwargs}
            )
            if model
            else None
        )
...
class EventDescriptor(pydantic.BaseModel):
    ...
    @classmethod
    def _normalize_sources(cls, value: Source) -> List[Union[None, StateIdentifier]]:
        identifiers = value if isinstance(value, list) else [value]
        return [identifier.name if isinstance(identifier, StateEnum) else identifier for identifier in identifiers]
...
def _state_action(
    name: Union[StateIdentifier, List[StateIdentifier]], type_: Action.Types, description: Optional[str] = None
):
    def decorator(fn):
...
def _summarize(values: Sequence[str], *, conjunction: str = "and", quote=False, oxford_comma: bool = True) -> str:
...
def get_instance_methods(obj, *, stop_at_parent: Optional[Type[Any]] = None) -> Dict[str, Callable]:
    ...
    if not inspect.isclass(obj):
        methods.maps.append(dict(filter(lambda item: inspect.ismethod(item[1]), obj.__dict__.items())))

    for c in inspect.getmro(cls):
        methods.maps.append(dict(filter(lambda item: inspect.isfunction(item[1]), c.__dict__.items())))
