...
class TestState:
    ...
    def test_add_action_invalid_type(self, state: statesman.State) -> None:
        with pytest.raises(
            ValueError,
            match=r'cannot add state action with type "(Types\.)?after": must be "(Types\.)?entry" or "(Types\.)?exit"',
        ):
...
class TestStateMachine:
    ...
    def test_repr(self, state_machine: statesman.StateMachine) -> None:
        assert (
            repr(state_machine)
            == "StateMachine(states=[State(name='starting', description='Starting'), State(name='running', description='Running'), State(name='stopping', description='Stopping'), State(name='stopped', description='Stopped')], events=[], state=None)"
        )
...
class TestTransition:
    ...
    async def test_lifecycle(self, transition: statesman.Transition) -> None:
        ...
        assert transition.cancelled is False
    ...
    class TestTrigger:
        ...
        class TestReturnTypes:
            ...
            async def test_return_types(
                self, state_machine: statesman.StateMachine, event, return_type, expected_return_value
            ) -> None:
                await state_machine.enter_state(States.starting)
                assert state_machine.state == States.starting
            ...
            @pytest.mark.asyncio
            async def test_return_types_override_on_trigger(
                self, state_machine: statesman.StateMachine, event, return_type, expected_return_value
            ) -> None:
                await state_machine.enter_state(States.starting)
                assert state_machine.state == States.starting
        ...
        @pytest.mark.asyncio
        async def test_cancel_via_guard_state_machine_method(
            self, state_machine: statesman.StateMachine, mocker
        ) -> None:
            await state_machine.enter_state(States.starting)
            with extra(state_machine):
        ...
        @pytest.mark.asyncio
        async def test_returning_none_from_guard_does_not_cancel(
            self, state_machine: statesman.StateMachine, mocker
        ) -> None:
            await state_machine.enter_state(States.starting)
            with extra(state_machine):
        ...
        @pytest.mark.asyncio
        async def test_returning_invalid_value_from_guard_raises_value_error(
            self, state_machine: statesman.StateMachine, mocker
        ) -> None:
            await state_machine.enter_state(States.starting)
            with extra(state_machine):
                ...
                with pytest.raises(
                    ValueError, match="invalid return value from guard_transition: must return True, False, or None"
                ):
                    await state_machine.trigger_event("finish")
                guard_mock.assert_awaited_once()
        ...
        async def test_non_assertion_errors_raise(self, state_machine: statesman.StateMachine, mocker) -> None:
            ...
            with extra(state_machine):
                guard_mock = mocker.patch.object(state_machine, "guard_transition")
                guard_mock.side_effect = RuntimeError("failed!")
        ...
        class TestActions:
            ...
            @pytest.mark.asyncio
            async def test_none_return_value_from_guard_does_not_cancel(
                self, state_machine: statesman.StateMachine, mocker
            ) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
            ...
            @pytest.mark.asyncio
            async def test_invalid_return_value_from_guard_raises_value_error(
                self, state_machine: statesman.StateMachine, mocker
            ) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
                ...
                event.add_action(lambda: guard_action(), statesman.Action.Types.guard)
                with pytest.raises(
                    ValueError, match="invalid return value from guard action: must return True, False, or None"
                ):
                    await state_machine.trigger_event("finish")
                guard_action.assert_called_once()

            @pytest.mark.asyncio
            async def test_cancel_via_guard_action_exception(
                self, state_machine: statesman.StateMachine, mocker
            ) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
...
class TestDecoratorStateMachine:
    ...
    class ProcessLifecycle(statesman.StateMachine):
        ...
        @statesman.event(source=States.stopped, target=States.stopped, transition_type=statesman.Transition.Types.self)
        async def halt(self) -> None:
            """Halt the process."""
            self.logs.append("Halted")
        ...
        async def _on_stop(self) -> None:
            self.logs.append("_on_stop triggered")
    ...
    @pytest.mark.asyncio
    async def test_trigger_event_through_method_call_with_args(
        self, state_machine: statesman.StateMachine, mocker
    ) -> None:
        with extra(state_machine):
...
class TestInitialState:
    ...
    def test_cannot_set_multiple_initial_states(self) -> None:
        with pytest.raises(
            ValueError, match='cannot declare more than one initial state: "States.one" already declared'
        ):
